      ...
      <div className="pb-6 px-4" onClick={(e) => e.stopPropagation()}>
        <div className="flex gap-2 p-2 bg-black/50 backdrop-blur-md rounded-xl max-w-[90vw] overflow-x-auto scrollbar-hide">
          {images.map((img, idx) => (
            ...
            >
              <img src={img} alt={imageAlts?.[idx] || ""} className="w-full h-full object-cover" />
            </button>
          ))}
        </div>
      </div>
...
export function FleetSection() {
  ...
  useGSAP(() => {
    if (infoRef.current) {
      ...
      gsap.fromTo(elements,
        ...
        { y: 0, opacity: 1, filter: 'blur(0px)', stagger: 0.08, duration: 0.5, ease: 'power2.out', clearProps: 'filter' }
      );
    }
  ...
  useGSAP(() => {
    ...
    gsap.fromTo(carouselRef.current,
      { opacity: 0, x: -50 },
      { opacity: 1, x: 0, duration: 0.8, ease: 'power3.out', scrollTrigger: { trigger: carouselRef.current, start: 'top 80%', once: true } }
    );
  }, { scope: containerRef });
  ...
  useGSAP(() => {
    ...
    cards.forEach(({ card, icon }) => {
      ...
      tl.to(card, { y: -4, duration: 0.3, ease: "power2.out" }).to(icon, { scale: 1.15, rotation: 5, duration: 0.3, ease: "back.out(1.7)" }, 0);
  ...
  const handleCardClick = (index: number) => {
    if (index === currentImageIndex) { setLightboxOpen(true); } else { setCurrentImageIndex(index); }
  };
  ...
  return (
    <section id="fleet" ref={containerRef} className="section-padding relative overflow-hidden bg-gray-50 dark:bg-gray-950 min-h-screen flex flex-col justify-center">
          ...
          <div ref={tablistRef} className="flex flex-wrap justify-center gap-2 md:gap-4 p-2 bg-white/40 dark:bg-white/5 backdrop-blur-2xl rounded-full border border-white/20 shadow-xl" role="tablist">
            {carCategories.map((cat) => {
              ...
              return (
                ...
                >
                  {activeCategory === cat.id && <div className="absolute inset-0 bg-gradient-to-r from-primary-600 to-primary-500 rounded-full -z-10" />}
          ...
          <div ref={infoRef} className="lg:col-span-4 flex flex-col gap-8 order-2 lg:order-1">
            ...
            <div className="info-anim pt-4">
              <button
                ...
                className="w-full group relative overflow-hidden flex items-center justify-between p-4 bg-gradient-to-r from-primary-600 via-primary-500 to-primary-600 text-white rounded-2xl font-bold shadow-xl transition-all duration-300 hover:-translate-y-1"
              >
                <span className="flex items-center gap-3 relative z-10">
                  <span className="w-10 h-10 rounded-full bg-white/20 flex items-center justify-center transition-transform duration-300 group-hover:rotate-45">
                    <ArrowRight className="w-5 h-5" />
                  </span>
                  عرض المعرض
                </span>
                <span className="font-mono opacity-80 text-sm relative z-10">{images.length} صور</span>
              </button>
            </div>
          </div>
      ...
      <Lightbox
        selectedImage={lightboxOpen ? images[currentImageIndex] : null} images={images} imageAlts={imageAlts} currentIndex={currentImageIndex}
        onClose={() => setLightboxOpen(false)} onNext={nextImage} onPrev={prevImage} onSelect={setCurrentImageIndex}
      />
    </section>
  );
}
